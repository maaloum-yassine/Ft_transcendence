...
def callback_42(request:Request):
    ...
    if 'error' in request.GET:
        ...
        response = redirect('https://127.0.0.1:/')
        return response
    ...
    if response.status_code != 200:
         response = redirect('https://127.0.0.1/404')
         return response
    ...
    if user :
        ...
        if user.active_2fa is True:
            ...
            response = redirect('https://127.0.0.1/twofa')
        else:
            user.is_online = True
            response = redirect('https://127.0.0.1/home')
        user.save()
    ...
    user.save()
    token = utils.generate_token(user, True)
    response = redirect('https://127.0.0.1:443/home')
    response.set_cookie(key ='jwt', value=token, httponly=True, secure=True, samesite='Strict')
