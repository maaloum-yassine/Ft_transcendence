const ChatManager = (() => {
  ...
  const createWebSocket = (url) => {
    ...
    // return new WebSocket('ws://localhost:8000/ws/second/');
  };
  ...
  // 	// Close the chatSocket when the page is about to be unloaded
  // 	if (chatSocket) {
  // 	  chatSocket.close();
  // 	}
  //   };
  ...
  const setupSocketEventListeners = (connectSocket) => {
    ...
    setupMessageInput();
    disableMessageInput(); // Disable message input initially
  };
  ...
  const setupConnectSocketMessages = (connectSocket) => {
    connectSocket.onmessage = (e) => {
      const data = JSON.parse(e.data);
      getElements("span.connected").forEach((paragraph) => {
        paragraph.style.display = "none";
      });

      data.connected_users.forEach((user) => {
        const paragraph = getElement(`span.connected${user.user_id}`);
        if (paragraph) {
          paragraph.style.display = "block";
        }
      });
  ...
  const cleanupWebSockets = () => {
    ...
    if (notificationSocket) {
      ...
      notificationSocket = null; // Clear the reference
    }
    console.log("WebSocket connections cleaned up.");
  };
  const handlePopState = () => {
    cleanupWebSockets(); // Close all WebSockets before handling the back navigation
    console.log("Navigated back, WebSocket connections closed.");
  };

  const handleBeforeUnload = () => {
    cleanupWebSockets(); // Close all WebSockets before the page is unloaded
  };
  ...
  const handleSuccessfulBlock = (friend, element) => {
    ...
    Array.from(parentElement.children).forEach((child) => {
      // Remove child if it's not the image
